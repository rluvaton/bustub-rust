...
impl ExecutorContext {
     ...
     */
    pub(crate) fn new(transaction: Arc<Transaction>, catalog: Arc<Catalog>, bpm: Arc<BufferPoolManager>, txn_mgr: Arc<TransactionManager>,
                      lock_mgr: Arc<LockManager>, is_delete: bool) -> Self {
        Self {
            transaction,
            catalog,
            bpm,
    ...
    /** @return the catalog */
    pub(crate) fn get_catalog(&self) -> &Arc<Catalog> { &self.catalog }
    ...
    pub(crate) fn get_lock_manager(&self) -> &Arc<LockManager> { &self.lock_mgr }

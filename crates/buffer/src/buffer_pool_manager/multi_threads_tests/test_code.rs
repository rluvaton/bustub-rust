...
pub fn run_multi_threads_tests(options: Options) {
    ...
    let bpm = std::sync::Arc::new(bpm);
    let page_ids = std::sync::Arc::new(parking_lot::RwLock::new(page_ids));
    ...
    for thread_id in 0..scan_thread_n {
        ...
        let page_ids = std::sync::Arc::clone(&page_ids);
...
fn init_buffer_pool_manager_for_test(options: &Options, temp_dir: Option<TempDir>) -> (Vec<PageId>, BufferPoolManager) {
    ...
    match options.disk_manager_specific.clone() {
        ...
        DiskManagerImplementationOptions::UnlimitedMemory(o) => unsafe {
            ...
            let mut manager_guard = cloned_manager.lock();

            manager_guard.enable_latency_simulator(o.enable_latency);
        }
    }

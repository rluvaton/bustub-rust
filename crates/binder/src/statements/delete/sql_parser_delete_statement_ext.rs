        ...
        let primary_columns_from_table_constraints: Vec<String> = {
            let primary_key_columns: ParseASTResult<Vec<&Vec<Ident>>> = self.constraints.iter().map(|constraint| {
                match constraint {
                    ...
                    _ => return Err(TryFromASTError::Unimplemented(format!("Constraints {} is not supported", constraint)))
                }
            }).collect();

            let primary_key_columns = primary_key_columns?;
            if primary_key_columns.len() > 1 {
                return Err(TryFromASTError::Unimplemented("Multiple primary key constraint on a single table is not supported".to_string()));
            }

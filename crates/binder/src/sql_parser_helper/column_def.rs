    ...
    fn try_convert_into_column(&self) -> ParseASTResult<Column> {
        ...
        let fixed_size_data_type = match self.data_type {
            DataType::CharacterVarying(size) | DataType::CharVarying(size) | DataType::Varchar(size) => {
                if size.is_none() {
                    return Err(TryFromASTError::Unimplemented("Unsupported size".to_string()))
                }
                    ...
                    CharacterLength::IntegerLength { length, unit } => {
                        match unit.unwrap_or(CharLengthUnits::Characters) {
                            ...
                            CharLengthUnits::Octets => {
                                return Err(TryFromASTError::Unimplemented("Unsupported unit of char size".to_string()))
                            }
                        }

                        length
                    }
                    CharacterLength::Max => return Err(TryFromASTError::Unimplemented("Should specify max length".to_string()))
                };
            ...
            _ => return Err(TryFromASTError::Unimplemented(format!("datatype {} is not supported", self.data_type)))
        };
        ...
        for option in &self.options {
            match option.option {
                ...
                ColumnOption::Unique { is_primary, .. } => {
                    if !is_primary {
                        return Err(TryFromASTError::Unimplemented("unique columns that are not primary key are not supported".to_string()))
                    }

                    is_col_primary = is_primary;
                },
                _ => return Err(TryFromASTError::Unimplemented(format!("Option {} is not supported ", option.option))),
            }
        }

...
impl DiskManager {
    ...
    pub(crate) fn new(db_file: PathBuf) -> Result<DiskManager> {
    ...
    pub fn read_page(&mut self, page_id: PageId, page_data: &mut [u8]) {
        ...
        if offset > Self::get_file_size(self.file_name.as_path()) {
     ...
     */
    pub fn write_log(&mut self, log_data: &[u8], size: i32) {
     ...
     */
    pub fn read_log(&mut self, log_data: &mut [u8], size: i32, offset: i32) -> bool {

        if offset > Self::get_file_size(self.log_name.as_path()) {
        ...
        if read_count < size as usize {
            ...
            // unsafe { write_bytes(&mut log_data[read_count..], 0, (size as usize) - read_count); }
        }
    ...
    */
     ...
     */
    pub fn set_flush_log_future(&mut self, f: Option<Box<dyn Future<Output=()>>>) {
        self.flush_log_f = f;
        unimplemented!()
    }
    ...
    fn get_file_size(file_name: &Path) -> i32 {
